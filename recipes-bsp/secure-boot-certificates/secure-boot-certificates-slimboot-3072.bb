...
do_compile(){
  for name in SigningKey; do
    openssl req -newkey rsa:3072 -nodes -keyout "${name}.key" -new -x509 -sha384 -days 3650 -subj "${SECURE_BOOT_CERT_GEN_SUBJECT}" -out "${name}.crt"
...
do_install(){
  ...
  for name in SigningKey; do
    for format in key pem crt cer; do
      install -m 644 ${name}.${format} ${D}/${datadir}/secure-boot-certificates-slimboot-3072
    done
  done
}
...
do_deploy(){
  ...
  for name in SigningKey; do
    for format in key pem crt cer; do
    install -m 644 ${name}.${format} ${DEPLOYDIR}/secure-boot-certificates-slimboot-3072
    done
  done
}
